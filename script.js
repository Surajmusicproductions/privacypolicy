// ======= SUPABASE AUTH + LOGIN-ONLY UNLOCK =======
let supabase = null;
let isAuthed = false;
...
function applyAuthGate() {
  ...
  // Hide “Go Premium” button (not used in login-only mode)
...
async function initAuth() {
  ...
  async function refreshHeader() {
    ...
    if (isAuthed) {
      ...
      // 🔑 Force unlock when logged in
      document.body.dataset.premium = "1";
      applyAuthGate();

      // Show “unlimited use” message in ad bar
      const badge = document.getElementById("ad-minutes-left");
      if (badge) badge.textContent = "✅ Logged in — unlimited use";
  ...
  signupBtn?.addEventListener("click", async () => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password: pass,
      options: {
        emailRedirectTo: "https://loopstation-pro.netlify.app/", // your deployed site
      },
    });
