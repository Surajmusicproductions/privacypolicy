...
(async ()=>{
  try {
    ...
    // ------------------------
    // Worklet recorder node pool (shared helper)
    // ------------------------
    const _recorderNodePool = []; // small pool for reuse
    function _createRecorderNode() {
      // create a node configured for stereo input, single input
      const node = new AudioWorkletNode(audioCtx, 'recorder-processor', {
        numberOfInputs: 1,
        numberOfOutputs: 0, // we are only using it for recording
        channelCount: 2,
        processorOptions: {}
      });

      // default safe handler: forward debug messages to console
      node.port.onmessage = (e) => {
        const d = e.data;
        if (!d) return;
        if (d.type === 'configured' || d.type === 'started' || d.type === 'stopped' || d.type === 'overdubStarted' || d.type === 'overdubStopped') {
          console.debug('RecorderWorklet:', d);
        } else if (d.type === 'dump') {
          // main handlers in Looper._stopWorkletRecording will set a temporary onmessage if they want detailed dump handling;
          // fallback: re-post to window so devtools can inspect
          window.__lastRecorderDump = d;
          console.log('Recorder dump available (raw):', d);
        } else {
          // generic debug
          console.debug('RecorderWorklet message:', d);
        }
      };

      // small convenience: configure defaults on creation
      node.port.postMessage({ cmd: 'configure', numTracks: 5, numChannels: 2, maxSecondsPerTrack: 120 });
      _recorderNodePool.push(node);
      return node;
    }

    function _getPooledRecorderNode() {
      // return existing unused node, or create one
      for (let n of _recorderNodePool) {
        if (!n._inUse) { n._inUse = true; return n; }
      }
      const newNode = _createRecorderNode();
      newNode._inUse = true;
      return newNode;
    }

    function _releasePooledRecorderNode(node) {
      try { node._inUse = false; } catch (e) { console.warn('releasePooledRecorderNode failed', e); }
    }

    // Make helpers available globally for the Looper class to use
    window._getPooledRecorderNode = _getPooledRecorderNode;
    window._releasePooledRecorderNode = _releasePooledRecorderNode;
...
class Looper {
  constructor(index, recordKey, stopKey){
    ...
    this._workletStopTimer = null; // for scheduled stop
  ...
  async _startWorkletRecording(lenSec){
    ...
    // Use pooled recorder node (safer for mobile and repeated recordings)
    try {
      // acquire a pooled node
      this.workletNode = _getPooledRecorderNode();

      // ensure it's marked as in-use for this Looper
      this.workletNode._ownerLooper = this.index;

      // connect the correct input (processed or dry) robustly
      try {
        ...
        if (shouldRecordProcessed) {
          // prefer fxSumGain; if that fails fallback to dryGain
          try { fxSumGain.connect(this.workletNode); }
          catch (e) { console.warn('fxSumGain.connect failed, falling back to dryGain:', e); dryGain.connect(this.workletNode); }
        } else {
          dryGain.connect(this.workletNode);
        }
      } catch (e) {
        console.warn('Recorder worklet connect failed, final fallback to dryGain:', e);
        try { dryGain.connect(this.workletNode); } catch (e2) { console.error('Fallback connect failed', e2); }
      }

      // mark worklet in-use and reset its internal buffers
      try { this.workletNode.port.postMessage({ cmd: 'reset' }); } catch(e){ console.warn('postMessage reset failed', e); }

      // schedule stop after length (keeps current behavior)
      this._workletStopTimer = setTimeout(()=> this._stopWorkletRecording(), lenSec * 1000);
    } catch (err) {
      // Worklet failed — keep the existing visual fallback behavior (do not throw)
      console.warn('AudioWorkletNode (pooled) creation failed; running visual fallback for recording:', err);
      showMsg('⚠️ Recorder worklet failed — visual record will run, but audio capture may not work.', '#ffe066');
      // schedule visual fallback stop so the ring completes
      setTimeout(() => {
  ...
  async _stopWorkletRecording(){
    if (this._workletStopTimer) { clearTimeout(this._workletStopTimer); this._workletStopTimer = null; }
    if (!this.workletNode) return;
    this.state = 'playing'; this.updateUI();

    const node = this.workletNode;

    // install a one-time dump handler (keeps other general messages unaffected)
    const dumpHandler = (e) => {
      const d = e.data;
      if (!d) return;
      if (d.type === 'dump' || d.cmd === 'dump') {
        // normalize worklet dump shape: accept channels[] OR interleaved buffer (d.buffer)
        let channels = d.channels || d.bufferChannels || null;
        const length = d.frames || d.length || d.framesLength || 0;
        const sampleRate = d.sampleRate || audioCtx.sampleRate;

        if ((!channels || !channels.length) && d.buffer && length && d.channels) {
          // de-interleave interleaved Float32Array 'buffer' -> channels[]
          const inter = d.buffer;
          const framesCount = length;
          const chCount = d.channels;
          channels = new Array(chCount);
          for (let c = 0; c < chCount; c++) channels[c] = new Float32Array(framesCount);
          for (let i = 0; i < framesCount; i++) {
            const base = i * chCount;
            for (let c = 0; c < chCount; c++) {
              channels[c][i] = inter[base + c];
            }
          }
        }

        if (!length || !channels || channels.length === 0) {
          // empty recording -> cleanup and release node
          try { node.port.onmessage = null; } catch (e) {}
          try { dryGain.disconnect(node); fxSumGain.disconnect(node); } catch (e) {}
          _releasePooledRecorderNode(node);
          this.workletNode = null;
          return;
        }

        try {
          // Build AudioBuffer from channels (channels[] is array of Float32Array)
          const chCount = channels.length;
          const audioBuf = audioCtx.createBuffer(chCount, length, sampleRate);
          for (let ch = 0; ch < chCount; ch++) {
            audioBuf.getChannelData(ch).set(channels[ch].subarray(0, length));
          }

          // determine targetSamples (preserve existing behavior)
          const sr = sampleRate;
          let targetSamples;
          if (this.index === 1) {
            const maxSamples = Math.round((this.targetLenSec || 60) * sr);
            targetSamples = Math.min(length, maxSamples);
          } else {
            targetSamples = Math.round(masterLoopDuration * sr * this.divider);
          }

          // if sizes differ, slice/resize into outBuffer
          let outBuffer;
          if (audioBuf.length !== targetSamples) {
            outBuffer = audioCtx.createBuffer(audioBuf.numberOfChannels, targetSamples, sr);
            for (let ch = 0; ch < audioBuf.numberOfChannels; ch++) {
              outBuffer.getChannelData(ch).set(audioBuf.getChannelData(ch).subarray(0, targetSamples));
            }
          } else {
            outBuffer = audioBuf;
          }

          // assign loop buffer and update state
          this.loopBuffer = outBuffer;
          this.loopDuration = outBuffer.duration;
          ...
          // cleanup: disconnect and release node back to pool
          try { dryGain.disconnect(node); fxSumGain.disconnect(node); node.disconnect(); } catch (e) {}
          try { node.port.onmessage = null; } catch (e) {}
          _releasePooledRecorderNode(node);
          this.workletNode = null;
          ...
          console.error('Error processing dump from recorder worklet:', err);
          try { node.port.onmessage = null; } catch (e) {}
          try { dryGain.disconnect(node); fxSumGain.disconnect(node); } catch (e) {}
          _releasePooledRecorderNode(node);
          this.workletNode = null;
        ...
        // Build AudioBuffer from channels (channels may already be Float32Array)
        try {
          const chCount = channels.length;
          const buffer = audioCtx.createBuffer(chCount, length, sampleRate);
          for (let ch = 0; ch < chCount; ch++) {
            // channels[ch] may already be a Float32Array
            buffer.getChannelData(ch).set(channels[ch].subarray(0, length));
          }

          // Determine targetSamples like previous logic (preserve your existing behavior)
          const sr = sampleRate;
          let targetSamples;
          if (this.index === 1) {
            const maxSamples = Math.round((this.targetLenSec || 60) * sr);
          ...
          // If length != targetSamples, slice
          let outBuffer;
          if (buffer.length !== targetSamples) {
            outBuffer = audioCtx.createBuffer(buffer.numberOfChannels, targetSamples, sr);
            for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
              outBuffer.getChannelData(ch).set(buffer.getChannelData(ch).subarray(0, targetSamples));
            }
          } else outBuffer = buffer;
    ...
    // set the handler (works on older browsers): replace any previous onmessage to avoid clobbering other consumers
    try {
      node.port.onmessage = dumpHandler;
    } catch (e) {
      node.port.addEventListener('message', dumpHandler);
    }

    // request dump
    try { node.port.postMessage({ cmd: 'dump', track: this.index }); } catch(e){ console.warn('dump postMessage failed', e); }
  }
...
abortRecording(){
    if (this.workletNode) {
        ...
        _releasePooledRecorderNode(this.workletNode);
        this.workletNode = null;
    }
